...
public partial class GameConductor : MonoBehaviour
{
    ...
    private bool timeShowingColon = true;
    private IEnumerator TimeUpdater()
    {
        while (true)
        {
            DataDump.Set("Time", DateTime.Now.ToShortTimeString().ToLower().Replace(":", timeShowingColon ? ":" : "<color=transparent>:</color>"));
            timeShowingColon = !timeShowingColon;
